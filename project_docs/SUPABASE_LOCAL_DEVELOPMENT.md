...
no public/private schema errors and all 167 pgTAP assertions passed. The suite
includes transaction-scoped authenticated sessions for cross-role, reporting
hierarchy, cross-market, inactive-user, preferences, result, enrollment, and
audited profile-management behavior. A real
...
The dry run must list only the 12 forward migrations. Any other result is a stop
condition.
