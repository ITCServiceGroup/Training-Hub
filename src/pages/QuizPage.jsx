...
const QuizPage = () => {
  ...
  const decodeAccessCode = (value) => {
    if (typeof value !== "string") return "";
    try {
      return decodeURIComponent(value).trim().toUpperCase().slice(0, 12);
    } catch {
      return "";
    }
  };
  ...
  const [localAccessCode, setLocalAccessCode] = useState(() =>
    decodeAccessCode(accessCode || location.state?.accessCode),
  );
  ...
  useEffect(() => {
    const routedCode = decodeAccessCode(accessCode);
    const stateCode = decodeAccessCode(location.state?.accessCode);
    const credential = routedCode || stateCode;
    if (credential) setLocalAccessCode(credential);

    if (accessCode) {
      navigate("/quiz/access", {
        replace: true,
        state: { accessCode: credential },
      });
    } else if (location.state?.accessCode) {
      navigate("/quiz/access", { replace: true, state: null });
    }
  }, [accessCode, location.state, navigate]);
  ...
  useEffect(() => {
    ...
    if (!quizId && !localAccessCode) {
      fetchSections();
    }
  }, [quizId, localAccessCode]);
      ...
      try {
        ...
        await Promise.all(
          categories.map(async (category) => {
            const count = await quizzesService.countPracticeQuizzesByCategory(
              category.id,
            );
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      ...
      try {
        ...
        const filteredQuizzes = data.filter((quiz) => {
          const categoryIds =
            typeof quiz.category_ids === "string"
    ...
    if (!quizId && !localAccessCode && categoryId) {
      fetchQuizzes();
    }
  }, [quizId, localAccessCode, categoryId]);

  // Handle access code submission
  const handleAccessCodeSubmit = (e) => {
    e.preventDefault();
    if (localAccessCode) {
      navigate("/quiz/access", { state: { accessCode: localAccessCode } });
    }
  };
  ...
  const handleQuizSelect = (quiz) => {
    ...
    if (sectionId && categoryId) {
      navigate(
        `/quiz/${quiz.id}?from=practice&sectionId=${sectionId}&categoryId=${categoryId}`,
      );
    } else {
  ...
  const groupedQuizzes = useMemo(() => {
    ...
    const bySection = groupBy(filteredQuizzes, (quiz) => {
      ...
      const categoryWithSection = quiz.categories?.find(
        (cat) => cat.section?.id,
      );
    ...
    for (const sectionId in bySection) {
      ...
      const sectionInfo = sectionQuizzes[0]?.categories?.find(
        (cat) => cat.section?.id === sectionId,
      )?.section;
      ...
      fullyGrouped[sectionId] = {
        ...
        categories: groupBy(sectionQuizzes, (quiz) => {
          ...
          const primaryCategory =
            quiz.categories?.find((cat) => cat.section_id === sectionId) ||
            quiz.categories?.[0];
    ...
    for (const sectionId in fullyGrouped) {
      for (const categoryId in fullyGrouped[sectionId].categories) {
        const quizzesInCategory =
          fullyGrouped[sectionId].categories[categoryId];
        const categoryInfo = quizzesInCategory[0]?.categories?.find(
  ...
  if (quizId || localAccessCode) {
    return (
      <div className="py-2 w-full flex flex-col flex-1">
        <div className="py-3 px-8">
          <div className="flex items-center gap-2">
            <Link to="/quiz" className="no-underline">
              <h2
                className={`text-3xl ${isDark ? "text-primary-light" : "text-primary-dark"} m-0 hover:opacity-90 transition-opacity`}
        ...
        <QuizTaker quizId={quizId} accessCode={localAccessCode || null} />
      </div>
    );
  }
      ...
      <div className="py-3 px-8">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center max-w-md w-full relative">
            ...
            {isSearching && (
              <div
                className={`absolute ${searchQuery ? "right-8" : "right-3"} top-1/2 -translate-y-1/2`}
              >
                <div
                  className={`w-4 h-4 rounded-full border-2 ${isDark ? "border-gray-700 border-t-primary" : "border-gray-200 border-t-primary"} animate-spin`}
                ></div>
              </div>
            )}
          </div>
      ...
      {searchResults && searchQuery ? (
        ...
        <>
          ...
          <div className="max-w-2xl mx-auto mb-6 mt-4">
            <div
              className={`${isDark ? "bg-slate-800 border-slate-700" : "bg-slate-50 border-slate-200"} rounded-lg p-5 border`}
            >
              <h2
                className={`text-xl font-bold ${isDark ? "text-white" : "text-slate-900"} mb-2`}
              >
                Have an Access Code?
              </h2>
              <p
                className={`${isDark ? "text-gray-300" : "text-slate-600"} mb-4 text-sm`}
              >
                If you have an access code for a specific quiz, enter it below
                to begin.
              </p>
              <form
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setLocalAccessCode(e.target.value.toUpperCase())
                  }
                  maxLength={12}
          ...
          {error && (
            <div
              className={`p-3 ${isDark ? "bg-red-900/30 border-red-900/50 text-red-400" : "bg-red-50 border-red-200 text-red-700"} border rounded-lg mb-2 text-sm`}
            >
              {error}
            </div>
          )}
          ...
          {!sectionId ? (
            ...
            <div className="px-8">
              <p
                className={`text-sm ${isDark ? "text-gray-300" : ""} mt-1 mb-2`}
              >
                Select a section below to view the practice quiz categories.
              </p>
              <SectionGrid
                sections={sections}
            ...
            <div className="px-8">
              ...
              <p
                className={`text-sm ${isDark ? "text-gray-300" : ""} mt-1 mb-2`}
              >
                Select a category below to view practice quizzes.
              </p>
              <CategoryGrid
            ...
            <div className="px-8">
              ...
              {isLoading ? (
                ...
                <div className="text-center py-6">
                  <p
                    className={`${isDark ? "text-gray-400" : "text-slate-600"} text-sm`}
                  >
                    No practice quizzes found for this category.
                  </p>
                </div>
              ) : (
                  ...
                  {quizzes.map((quiz) => (
                    ...
                    >
                      <div className="p-6 flex-grow">
                        ...
                        <h5
                          className={`text-lg font-bold ${isDark ? "text-white" : "text-slate-900"} mb-2`}
                        >
                          {quiz.title}
                        </h5>
                        {quiz.description && (
                          <p
                            className={`${isDark ? "text-gray-400" : "text-slate-600"} text-sm mb-4 line-clamp-3`}
                          >
                            {quiz.description}
                          </p>
                        )}
                        <div
                          className={`flex items-center gap-4 text-xs ${isDark ? "text-gray-500" : "text-slate-500"}`}
                        >
                          <span>
                            {quiz.questionCount || quiz.question_count || 0}{" "}
                            Questions
                          </span>
                          {quiz.time_limit && (
                            <span>
                              {Math.floor(quiz.time_limit / 60)} Min Limit
                            </span>
                          )}
                        </div>
                      </div>
                      <div
                        className={`px-6 py-4 ${isDark ? "bg-slate-700 border-slate-600" : "bg-slate-50 border-slate-200"} border-t`}
                      >
                        {quiz.is_practice ? (
                          <button
