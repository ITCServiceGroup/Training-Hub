...
class ExportService {
  ...
  async exportChartAsPNG(chartElement, filename = 'chart') {
    ...
    try {
      ...
      await this._ensureElementReady(chartElement);

      // Check if element contains SVG (common with chart libraries)
      const hasSVG = chartElement.querySelector('svg') !== null;

      let canvas;
      if (hasSVG) {
        console.log('📊 SVG detected, using dom-to-image for better SVG support');
        ...
        // Convert data URL to canvas for consistent handling
        canvas = await this._dataUrlToCanvas(dataUrl);
      } else {
        // Use html2canvas for non-SVG content
        canvas = await html2canvas(chartElement, {
          ...EXPORT_CONFIG.png,
          width: EXPORT_CONFIG.chart.width,
          height: EXPORT_CONFIG.chart.height
        });
      }
      ...
      console.log('✅ Chart PNG export completed');
      return true;
    } catch (error) {
  ...
  async exportChartAsPDF(chartElement, filename = 'chart') {
    ...
    try {
      ...
      let canvas;
      if (hasSVG) {
        console.log('📊 PDF: SVG detected, using dom-to-image');
        // Use dom-to-image for SVG-heavy content
        const dataUrl = await domtoimage.toPng(chartElement, {
          quality: 1.0,
          bgcolor: '#ffffff',
        ...
        // Create canvas from chart using html2canvas
        canvas = await html2canvas(chartElement, EXPORT_CONFIG.png);
      }
      ...
      // Center the image vertically if it's smaller than page
      ...
      console.log('✅ Chart PDF export completed');
      return true;
    } catch (error) {
