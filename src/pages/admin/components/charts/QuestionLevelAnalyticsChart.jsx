...
const QuestionLevelAnalyticsChart = ({ data = [], loading = false }) => {
  ...
  const [questionState, setQuestionState] = useState(() => {
    ...
    if (cached && now - cached.timestamp < GLOBAL_CACHE_DURATION) {
      console.log("💾 QuestionAnalytics: Found cached data on mount!", {
        signature,
        cachedDataLength: cached.data.length,
      });
      return {
        data: cached.data,
        loading: false,
  ...
  const { getFiltersForChart, shouldFilterChart, drillDown, applyHoverFilter } =
    useDashboard();
  ...
  const chartFilteredData = useMemo(() => {
    ...
    return data.filter((result) => {
      if (filters.supervisor && result.supervisor !== filters.supervisor)
        return false;
      ...
      if (filters.quizType && result.quiz_type !== filters.quizType)
        return false;
      return true;
    });
  ...
  useEffect(() => {
    ...
    if (!hasDataChanged && questionState.hasEverLoaded) {
      console.log(
        "⏭️ QuestionAnalytics: Skipping fetch - no data change and already loaded",
        {
          hasDataChanged,
  ...
  const tooltipFunction = useCallback(({ id, value, data }) => {
    const tooltipData = [
      ...
      {
        label: "Avg Time",
        value: data.avgTimeSpent ? `${data.avgTimeSpent}s` : "No data",
      },
    ...
    if (data.isLegacyNotice) {
      additionalInfo =
        "Legacy quiz results do not contain question-level response data needed for this analysis.";
      ...
      additionalInfo = data.needsReview
        ? "This question may need review or revision"
        : "Question performance is within normal range";
    } else {
    ...
    if (data.isLegacyData) {
      additionalInfo +=
        " | Note: This data is estimated from historical quiz results.";
    } else if (data.isSimulatedData) {
      additionalInfo +=
        " | Note: This data is simulated based on overall quiz scores.";
    }
  ...
  if (loading || questionState.loading) {
    console.log("📊 QuestionAnalytics: Showing loading state", {
      loading,
  ...
  console.log("📊 QuestionAnalytics: Rendering chart with data", {
