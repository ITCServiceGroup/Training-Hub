...
const routerSimulatorAssetVersion = new URL(import.meta.url).searchParams.get(
  "v",
);
...
const DEFAULT_PLACEMENT_ADVICE =
  "Drag the router to test different positions in this home layout. The heatmap shows signal strength considering walls, appliances, and active interference sources.";
...
class RouterSimulatorElement extends HTMLElement {
  constructor() {
    ...
    this.shadowRoot.appendChild(
      routerSimulatorTemplate.content.cloneNode(true),
    );
    ...
    this.meshExtenderButton =
      this.shadowRoot.getElementById("meshExtenderButton"); // Button 1
    this.meshExtender2 = this.shadowRoot.getElementById("meshExtender2"); // Extender 2
    this.meshExtenderButton2 = this.shadowRoot.getElementById(
      "meshExtenderButton2",
    ); // Button 2
    this.signalStrengthCanvas =
      this.shadowRoot.getElementById("signalStrength");
    // Initialize canvas context with error handling
    if (this.signalStrengthCanvas) {
      const ctx = this.signalStrengthCanvas.getContext("2d", {
    ...
    this.fixedObstaclesContainer =
      this.shadowRoot.getElementById("fixedObstacles");
    this.interferenceContainer = this.shadowRoot.getElementById(
      "interferenceSources",
    );
    ...
    this.babyMonitorToggle =
      this.shadowRoot.getElementById("babyMonitorToggle");
    ...
    this.rotateFurnitureLeftButton = this.shadowRoot.getElementById(
      "rotateFurnitureLeftButton",
    );
    this.rotateFurnitureRightButton = this.shadowRoot.getElementById(
      "rotateFurnitureRightButton",
    );
    this.copyFurnitureJsonButton = this.shadowRoot.getElementById(
      "copyFurnitureJsonButton",
    );
    this.saveFurnitureButton = this.shadowRoot.getElementById(
      "saveFurnitureButton",
    );
    this.selectedFurnitureLabel = this.shadowRoot.getElementById(
      "selectedFurnitureLabel",
    );
    this.selectedFurnitureMeta = this.shadowRoot.getElementById(
    ...
    this.floorplanData = JSON.parse(
      JSON.stringify(allFloorplans[this.currentFloorplanKey]),
    );
  ...
  _attachEventListeners() {
    ...
    this.router.addEventListener("mousedown", (e) =>
      dragHandlers.startDrag(e, this),
    );
    this.router.addEventListener(
      "touchstart",
      (e) => dragHandlers.startDrag(e, this),
      { passive: false },
    );
    // Use document for move/end events to handle dragging outside the element
    document.addEventListener(
      "mousemove",
      (this._handleMoveRouter = (e) => dragHandlers.moveRouter(e, this)),
    );
    document.addEventListener(
      "mouseup",
      (this._handleStopDrag = (e) => dragHandlers.stopDrag(e, this)),
    );
    document.addEventListener("touchmove", this._handleMoveRouter, {
      passive: false,
    }); // Re-use bound handler
    ...
    this.meshExtender.addEventListener("mousedown", (e) =>
      dragHandlers.startMeshExtenderDrag(e, this, 1),
    );
    this.meshExtender.addEventListener(
      "touchstart",
      (e) => dragHandlers.startMeshExtenderDrag(e, this, 1),
      { passive: false },
    );
    this.meshExtender2.addEventListener("mousedown", (e) =>
      dragHandlers.startMeshExtenderDrag(e, this, 2),
    );
    this.meshExtender2.addEventListener(
      "touchstart",
      (e) => dragHandlers.startMeshExtenderDrag(e, this, 2),
      { passive: false },
    );
    ...
    document.addEventListener(
      "mousemove",
      (this._handleMoveMeshExtender = (e) =>
        dragHandlers.moveMeshExtender(e, this)),
    );
    document.addEventListener(
      "mouseup",
      (this._handleStopMeshExtenderDrag = (e) =>
        dragHandlers.stopMeshExtenderDrag(e, this)),
    );
    document.addEventListener("touchmove", this._handleMoveMeshExtender, {
      passive: false,
    }); // Re-use bound handler
    ...
    this.meshExtenderButton.addEventListener(
      "click",
      this._toggleMeshExtender.bind(this, 1),
    );
    this.meshExtenderButton2.addEventListener(
      "click",
      this._toggleMeshExtender.bind(this, 2),
    );
    this.bluetoothToggle.addEventListener(
      "click",
      this._toggleInterference.bind(this, "bluetooth"),
    );
    this.babyMonitorToggle.addEventListener(
      "click",
      this._toggleInterference.bind(this, "babyMonitor"),
    );
    // Floorplan tab clicks
    if (this.floorplanTabs) {
      this.floorplanTabs.addEventListener(
        "click",
        this._handleFloorplanChange.bind(this),
      );
    }
    ...
    if (this.isDevMode) {
      document.addEventListener(
        "mousemove",
        (this._handleMoveFurniture = (e) => this._moveFurniture(e)),
      );
      document.addEventListener(
        "mouseup",
        (this._handleStopFurnitureDrag = () => this._stopFurnitureDrag()),
      );
      window.addEventListener(
        "keydown",
        (this._handleFurnitureKeyDown = (e) =>
          this._handleFurnitureEditorKeyDown(e)),
      );

      if (this.rotateFurnitureLeftButton) {
        this.rotateFurnitureLeftButton.addEventListener(
          "click",
          (this._handleRotateFurnitureLeft = () =>
            this._rotateSelectedFurniture(-90)),
        );
      }
      if (this.rotateFurnitureRightButton) {
        this.rotateFurnitureRightButton.addEventListener(
          "click",
          (this._handleRotateFurnitureRight = () =>
            this._rotateSelectedFurniture(90)),
        );
      }
      if (this.copyFurnitureJsonButton) {
        this.copyFurnitureJsonButton.addEventListener(
          "click",
          (this._handleCopyFurnitureJson = () => this._copyFurnitureJson()),
        );
      }
      if (this.saveFurnitureButton) {
        this.saveFurnitureButton.addEventListener(
          "click",
          (this._handleSaveFurniture = () => this._saveFurnitureToSource()),
        );
      }
    }
  }
  ...
  _removeEventListeners() {
    ...
    if (this._handleMoveRouter)
      document.removeEventListener("mousemove", this._handleMoveRouter);
    if (this._handleStopDrag)
      document.removeEventListener("mouseup", this._handleStopDrag);
    if (this._handleMoveRouter)
      document.removeEventListener("touchmove", this._handleMoveRouter);
    if (this._handleStopDrag)
      document.removeEventListener("touchend", this._handleStopDrag);
    if (this._handleStopDrag)
      document.removeEventListener("touchcancel", this._handleStopDrag);

    // Mesh extender drag events - Remove the handlers stored during addEventListener
    if (this._handleMoveMeshExtender)
      document.removeEventListener("mousemove", this._handleMoveMeshExtender);
    if (this._handleStopMeshExtenderDrag)
      document.removeEventListener("mouseup", this._handleStopMeshExtenderDrag);
    if (this._handleMoveMeshExtender)
      document.removeEventListener("touchmove", this._handleMoveMeshExtender);
    if (this._handleStopMeshExtenderDrag)
      document.removeEventListener(
        "touchend",
        this._handleStopMeshExtenderDrag,
      );
    if (this._handleStopMeshExtenderDrag)
      document.removeEventListener(
        "touchcancel",
    ...
    if (this._handleMoveInterference)
      document.removeEventListener("mousemove", this._handleMoveInterference);
    if (this._handleStopInterference)
      document.removeEventListener("mouseup", this._handleStopInterference);
    if (this._handleMoveInterference)
      document.removeEventListener("touchmove", this._handleMoveInterference);
    if (this._handleStopInterference)
      document.removeEventListener("touchend", this._handleStopInterference);
    if (this._handleStopInterference)
      document.removeEventListener("touchcancel", this._handleStopInterference);

    if (this._handleMoveFurniture)
      document.removeEventListener("mousemove", this._handleMoveFurniture);
    if (this._handleStopFurnitureDrag)
      document.removeEventListener("mouseup", this._handleStopFurnitureDrag);
    if (this._handleFurnitureKeyDown)
      window.removeEventListener("keydown", this._handleFurnitureKeyDown);
    ...
    if (this.worker) {
      this.worker.removeEventListener(
        "message",
        this._boundHandleWorkerMessage,
      );
    }

    if (this.rotateFurnitureLeftButton && this._handleRotateFurnitureLeft) {
      this.rotateFurnitureLeftButton.removeEventListener(
        "click",
        this._handleRotateFurnitureLeft,
      );
    }
    if (this.rotateFurnitureRightButton && this._handleRotateFurnitureRight) {
      this.rotateFurnitureRightButton.removeEventListener(
        "click",
        this._handleRotateFurnitureRight,
      );
    }
    if (this.copyFurnitureJsonButton && this._handleCopyFurnitureJson) {
      this.copyFurnitureJsonButton.removeEventListener(
        "click",
        this._handleCopyFurnitureJson,
      );
    }
    if (this.saveFurnitureButton && this._handleSaveFurniture) {
      this.saveFurnitureButton.removeEventListener(
        "click",
        this._handleSaveFurniture,
      );
    }
  }
  ...
  _initializeWorker() {
    try {
      ...
      console.log(
        `[Main] Current path: ${currentPath}, Base path: ${basePath}`,
      );
  ...
  _sendConfigToWorker() {
    ...
    if (!this.signalStrengthCanvas || !this.floorplanData) {
      console.warn(
        "[Main] _sendConfigToWorker: Canvas or floorplan data not ready.",
      );
      return;
    }
    ...
    if (canvasWidth > 0 && canvasHeight > 0) {
      ...
      if (!this.isWorkerReady) {
        ...
        console.log(
          "[Main] Worker marked as ready. Requesting initial signal calc.",
        );
        this._requestSignalUpdate(); // Trigger initial calculation now that config is sent
      }
    } else {
      console.warn(
        "[Main] Canvas dimensions are zero, skipping config update.",
      );
    }
  }
  ...
  _handleWorkerMessage(e) {
    ...
    if (type === "signalResult") {
      ...
      if (this.signalCtx && payload.imageData) {
        ...
        console.warn(
          "[Main] Canvas context or imageData missing, cannot draw heatmap.",
        );
      }
      ...
      if (this.pendingWorkerUpdate) {
        this.pendingWorkerUpdate = false;
        console.log(
          "[Main] Pending update exists, requesting new calculation.",
        );
  ...
  _drawHeatmap(imageData) {
    ...
    if (tempCtx) {
      ...
      console.warn(
        "[Main] _drawHeatmap: Failed to get context for temporary blur canvas.",
      );
  ...
  _handleFloorplanChange(event) {
    ...
    this.interferenceElements = initializeFloorplanVisuals(
      this.shadowRoot,
      this.floorplanData,
      this._getFloorplanVisualOptions(),
    );
  ...
  _reAttachInterferenceHandlers() {
    ...
    this.floorplanData.interferenceSources.forEach((source) => {
      ...
      if (element) {
        ...
        element.addEventListener("mousedown", (e) =>
          dragHandlers.startInterferenceDrag(e, this, source.name),
        );
        element.addEventListener(
          "touchstart",
          (e) => dragHandlers.startInterferenceDrag(e, this, source.name),
          { passive: false },
        );
      }
    });
    ...
    document.addEventListener(
      "mousemove",
      (this._handleMoveInterference = (e) =>
        dragHandlers.moveInterference(e, this)),
    );
    document.addEventListener(
      "mouseup",
      (this._handleStopInterference = (e) =>
        dragHandlers.stopInterferenceDrag(e, this)),
    );
    document.addEventListener("touchmove", this._handleMoveInterference, {
      passive: false,
    });
  ...
  _resetDevicePositions() {
    ...
    if (
      ...
      initialX = Math.round(
        firstRoom.x + firstRoom.width / 2 - this.routerDimensions.width / 2,
      );
      initialY = Math.round(
        firstRoom.y + firstRoom.height / 2 - this.routerDimensions.height / 2,
      );
    } else {
      initialX = 50;
      initialY = 50;
    ...
    this.extenders.forEach((ext) => {
      if (ext.placed) {
        const bestPosition = this._findBestMeshExtenderPosition(
          initialX,
          initialY,
          ext.id,
        );
  ...
  _syncFloorplanTabs() {
    ...
    this.floorplanTabs.querySelectorAll(".tab-button").forEach((btn) => {
      btn.classList.toggle(
        "active",
        btn.dataset.floorplan === this.currentFloorplanKey,
      );
    });
  }
  ...
  _refreshFurnitureEditorBindings() {
    ...
    this.furnitureElements = Array.from(
      this.furnitureContainer.querySelectorAll(
        '[data-editor-kind="furniture"]',
      ),
    );
    this.obstacleElements = Array.from(
      this.fixedObstaclesContainer.querySelectorAll(
        '[data-editor-kind="obstacle"]',
      ),
    );
    ...
    this.furnitureElements.forEach((element) => {
      ...
      element.addEventListener("mousedown", (event) =>
        this._beginEditorDrag(event, "furniture", furnitureIndex),
      );
    });
    ...
    this.obstacleElements.forEach((element) => {
      ...
      element.addEventListener("mousedown", (event) =>
        this._beginEditorDrag(event, "obstacle", obstacleIndex),
      );
    });
  ...
  _getEditorElement(kind, index) {
    ...
    return (
      elements.find(
        (item) => parseInt(item.dataset[datasetKey], 10) === index,
      ) || null
    );
  }
  ...
  _syncFurnitureSelection() {
    ...
    this.furnitureElements.forEach((element) => {
      ...
      if (
        this.selectedEditorItem?.kind !== "furniture" ||
        furnitureIndex !== this.selectedEditorItem.index
      ) {
        return;
      }
    ...
    this.obstacleElements.forEach((element) => {
      const obstacleIndex = parseInt(element.dataset.obstacleIndex, 10);
      const isSelected =
        this.selectedEditorItem?.kind === "obstacle" &&
        obstacleIndex === this.selectedEditorItem.index;
  ...
  _updateSelectedFurnitureMeta() {
    ...
    if (this.selectedFurnitureLabel) {
      this.selectedFurnitureLabel.textContent =
        kind === "furniture"
          ? `Selected: ${item.type} (${item.room})`
          : `Selected: ${item.type} (fixed obstacle)`;
    }
    if (this.selectedFurnitureMeta) {
      this.selectedFurnitureMeta.textContent =
        kind === "furniture"
  ...
  _applyObstacleStyles(obstacle, element) {
    ...
    element.style.transform = obstacle.rotation
  ...
  _moveFurniture(event) {
    ...
    const parentRect = this.routerPlacement.getBoundingClientRect();
    let newX = Math.round(
      event.clientX - parentRect.left - this.editorDragOffsetX,
    );
    let newY = Math.round(
      event.clientY - parentRect.top - this.editorDragOffsetY,
    );

    newX = Math.max(
      0,
      Math.min(newX, this.routerPlacement.offsetWidth - item.width),
    );
    newY = Math.max(
      0,
      Math.min(newY, this.routerPlacement.offsetHeight - item.height),
    );
  ...
  _rotateSelectedFurniture(delta) {
    ...
    this._updateSelectedFurnitureMeta();
    this._setDevStatus(
      `Rotated ${item.type} to ${item.rotation} degrees.`,
      "success",
    );
  }
  ...
  _nudgeSelectedFurniture(deltaX, deltaY) {
    ...
    item.x = Math.max(
      0,
      Math.min(item.x + deltaX, this.routerPlacement.offsetWidth - item.width),
    );
    item.y = Math.max(
      0,
      Math.min(
        item.y + deltaY,
        this.routerPlacement.offsetHeight - item.height,
      ),
    );
  ...
  async _saveFurnitureToSource() {
    ...
    try {
      const tokenResponse = await fetch("/__router-simulator/source-token", {
        cache: "no-store",
      });
      if (!tokenResponse.ok) {
        throw new Error("The local source-write token is unavailable.");
      }
      ...
      this._setDevStatus(
        `Saved ${this.currentFloorplanKey} layout to floorplan.js.`,
        "success",
      );
    } catch (error) {
  ...
  _updatePlacementAdvice() {
    ...
    this.placementAdvice.textContent =
      this.floorplanData?.placementAdvice || DEFAULT_PLACEMENT_ADVICE;
  }
  ...
  _updateCanvasDimensions() {
    ...
    if (containerWidth > 0 && containerHeight > 0) {
      ...
      console.log(
        `[Main] Canvas dimensions updated: ${containerWidth}x${containerHeight}`,
      );
      ...
      console.warn(
        "[Main] Router placement container has zero dimensions, skipping canvas update.",
      );
    }
  }
  ...
  _toggleMeshExtender(extenderId) {
    ...
    console.log(
      `[Main] Toggled mesh extender ${extenderId}: ${extender.placed}`,
    );
  ...
  _findBestMeshExtenderPosition(routerX, routerY, extenderId) {
    const extenderPlacement =
      this.floorplanData?.extenderPlacements?.[extenderId];
    let targetRoomName = extenderPlacement?.room || "Living Room";
    if (!extenderPlacement && extenderId === 2) {
      targetRoomName = this.floorplanData.rooms.some(
        (r) => r.name === "Bedroom 1",
      )
        ? "Bedroom 1"
        : "Dining Room";
    ...
    const targetRoom = this.floorplanData.rooms.find(
      (r) => r.name === targetRoomName,
    );
    const extender = this.extenders.find((ext) => ext.id === extenderId);
    const dimensions = extender
      ? extender.dimensions
      : { width: 60, height: 60 }; // Fallback dimensions

    if (targetRoom) {
      const anchorX = Number.isFinite(extenderPlacement?.anchorX)
        ? extenderPlacement.anchorX
        : extenderId === 1
          ? 0.66
          : 0.5;
      const anchorY = Number.isFinite(extenderPlacement?.anchorY)
        ? extenderPlacement.anchorY
        : 0.5;
      let posX =
        targetRoom.x +
        Math.round(targetRoom.width * anchorX) -
        dimensions.width / 2;
      let posY =
        targetRoom.y +
        Math.round(targetRoom.height * anchorY) -
        dimensions.height / 2;
      ...
      if (this.routerPlacement) {
        posX = Math.round(
          Math.max(
            0,
            Math.min(posX, this.routerPlacement.offsetWidth - dimensions.width),
          ),
        );
        posY = Math.round(
          Math.max(
            ...
            Math.min(
              posY,
              this.routerPlacement.offsetHeight - dimensions.height,
            ),
          ),
        );
      }
    ...
    return {
      x: routerX + (extenderId === 1 ? 200 : -100),
      y: routerY + (extenderId === 1 ? 0 : 100),
    };
  }

  // --- Interference Handlers --- (Keep this logic in the main class)
  _toggleInterference(sourceName) {
    const sourceData = this.floorplanData.interferenceSources.find(
      (s) => s.name === sourceName,
    );
    if (!sourceData) {
      return;
    ...
    if (button) {
      button.classList.toggle("active", sourceData.active);
    }
    ...
    console.log(
      `[Main] Toggled interference source '${sourceName}': ${sourceData.active}`,
    );
  ...
  isColorDark(color) {
    try {
      ...
      if (
        ...
        color.includes("rgb(243, 244, 246)")
      ) {
        // Gray 100
        return false;
      }
  ...
  setupThemeObserver() {
    ...
    this.messageHandler = (event) => {
      if (event.data && event.data.type === "theme-change") {
        console.log(
          "[RouterSimulator] Received theme change message:",
          event.data.theme,
        );
  ...
  applyTheme() {
    ...
    if (!isDarkMode && window !== window.parent) {
      try {
        ...
        if (
          ...
          const parentBodyBgColor = getComputedStyle(
            window.parent.document.body,
          ).backgroundColor;
          const parentHtmlBgColor = getComputedStyle(
            window.parent.document.documentElement,
          ).backgroundColor;

          if (
            this.isColorDark(parentBodyBgColor) ||
            this.isColorDark(parentHtmlBgColor)
          ) {
            isDarkMode = true;
          }
        }
        ...
        console.log(
          "[RouterSimulator] Could not access parent frame due to same-origin policy",
        );
      }
    }
    ...
    if (themeChanged) {
      console.log(
        "[RouterSimulator] Theme detection result:",
  ...
  updateTheme(isDarkMode) {
    console.log(
      "[RouterSimulator] updateTheme called with isDarkMode:",
      isDarkMode,
    );
    ...
    if (this.isTransitioning) {
      console.log(
        "[RouterSimulator] Theme transition already in progress, skipping",
      );
      return;
    }
